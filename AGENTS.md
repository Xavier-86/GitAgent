...
- macOS release: `build-release.sh` builds the Release app into `.build/release/` (`--install` also copies it to `/Applications`). A "Refresh .build release" script phase runs it automatically after every macOS Debug build, so the packaged app never goes stale (this is why `Build.xcconfig` sets `ENABLE_USER_SCRIPT_SANDBOXING = NO`).
...
- `Agent/Coder/` — Coder agent: manages interactive coding-CLI sessions (Kimi Code / Claude Code / Codex, **yolo mode** — explicitly user-requested, no confirmation loop) running detached in tmux on a connected `RepositoryLocation` working tree (`$HOME/.gitagent/coder/<session-uuid>/`, tmux session `ga-coder-<uuid>`). All tmux invocations use a dedicated server socket (`tmux -L gitagent`) so server options and session names never collide with the user's own tmux; the server sets `extended-keys on` + `extended-keys-format csi-u` because Kimi's TUI only submits on Enter with csi-u extended keys. `CoderExecutor` creates/probes/kills sessions over the shared SSH route or the local login shell; `CoderTerminalView` is a full xterm.js terminal that attaches to the tmux session (same wiring as the Terminal page) and pops back when it ends; on iOS it adds an optional chat-style message bar (on by default, toolbar toggle) that types the draft plus Return into the session, since driving a TUI from the raw terminal keyboard is painful on a phone. Completion detection uses per-invocation Stop/notify hooks appending to `turn-done`; Kimi receives a session-scoped `Stop` hook through an isolated, validated `KIMI_CODE_HOME` overlay so the user's global config is never modified, with tmux output idleness retained only for older Kimi versions that cannot load the hook. Exact hook counters are watched through a lightweight long-lived channel, while full tmux state remains on a five-second probe. A finished turn marks the list row and posts a local notification. `CoderStore` persists records to `coder-sessions.json` while the app runs.
- `Agent/RepoLaunch/` — Swift-native repository deployment: staged preflight/checkout/setup/build/test/verify workflow, tmux-detached executors on both targets (SSH hosts and the local Mac): a host-side tmux session runs all stages, the app polls `run.log`/status files and re-attaches after socket loss, app backgrounding, or app restart. Both targets therefore need `tmux` installed (macOS: Homebrew). Application Support history/log persistence, and separate form/history SwiftUI components. Successful deployments open Terminal at the verified working tree. Explicit setup/build/test commands are user-visible and user-triggered; a later remote coding CLI may propose them.
...
- `SSH/` — terminal + SSH transport, the **only** remote-execution channel: `SSHConnection` owns direct and multi-hop Citadel client chains; `SSHTerminalSession` opens the PTY shell; `LocalTerminalSession` provides the macOS login shell via `forkpty`; `SSHHostConfig`/`SSHHostStore` persist host metadata and resolve jump routes; `SSHEd25519Credential` generates app-owned keys; passwords and private keys stay in Keychain; `HostKeyStore` provides automatic TOFU exact-key pinning; `TerminalView` is the xterm.js WKWebView bridge; `TerminalLaunchCoordinator` routes repository locations and deployments; `RemoteDirectoryBrowser`/`RemotePathPickerView` browse remote paths; and `SSHView` owns the host list/editor/terminal UI. A saved target remains an independent device while optionally connecting through another saved host. The directly reached first hop may use password or Ed25519 authentication; every host reached through a jump route is forced to use its own Ed25519 key. Terminal, repository verification/browsing, and RepoLaunch all share this route. iOS uses SSH for LAN Mac/Linux; macOS uses SSH for other hosts and the native local shell for itself. Design notes and pending work (fingerprint confirmation UI, imported/encrypted private keys, SFTP) are in `SSH/SSH.md`.
- `Docs/` — design documents and developer records (docs only, no code yet):
  - `Docs/Development.md` (+ `Development.zh.md`) — developer-facing records moved out of the README: vision, detailed feature status, agent architecture decisions, roadmap, jump-host setup, and project structure. The READMEs stay user-facing; keep detailed records here.
...
- **Persistence tiers, strictly enforced:**
  - Secrets (GitHub OAuth token, LLM API keys, SSH host passwords, SSH Ed25519 private keys) → **system Keychain only**, never bundled or hardcoded. SSH private keys are generated on-device; passwords and API keys are user-entered.
  - App settings (font sizes, provider, base URL, model) and SSH host metadata (no passwords or private keys) → UserDefaults.
  - Chat sessions, repository locations, Coder session records, RepoLaunch deployment history/logs, and installed web assets → JSON/files under Application Support.
- **Navigation:** iPhone uses Safari-style `WorkspaceStore` pages, each owning one `NavigationStack`; do not switch to a collapsed split view on iPhone (it pops two levels per back swipe). Regular-width iOS and macOS use `NavigationSplitView` (`Views/MainView.swift`). The split detail column installs one fixed sidebar toggle via the `sidebarToggleButton()` modifier (defined in `MainView.swift`, binding passed through the `sidebarVisibility` environment value, which is injected on the `NavigationSplitView` itself — a value set inside the detail column reaches its root page but not pushed pages on macOS); toolbar items do not propagate to pushed pages, so every page that can be pushed into the detail stack must apply the modifier itself.
- **Web assets:** `Resources/web/` is installed to Application Support on first launch. When you change anything under `Resources/web/` (including the terminal assets), bump `WebAssets.version` in `Rendering/WebMarkdownView.swift` (currently `"14"`) to force re-installation. Note the file-system-synchronized group flattens resources into the bundle root — two files with the same basename anywhere under `GitAgent/` collide at build time.
...
- Do not store credentials anywhere but the Keychain (`Auth/KeychainHelper.swift`); UserDefaults and Application Support are for non-sensitive data only. SSH host passwords and Ed25519 private keys follow the same rule and are keyed by host UUID. OpenSSH public keys are non-secret and may be copied to a target host's `~/.ssh/authorized_keys`.
