...
- **Navigation:** iPhone uses a single `NavigationStack` deliberately — do not switch to a collapsed split view on iPhone (it pops two levels per back swipe). iPad/macOS use `NavigationSplitView` (`Views/MainView.swift`). The split detail column installs one fixed sidebar toggle via the `sidebarToggleButton()` modifier (defined in `MainView.swift`, binding passed through the `sidebarVisibility` environment value, which is injected on the `NavigationSplitView` itself — a value set inside the detail column reaches its root page but not pushed pages on macOS); toolbar items do not propagate to pushed pages, so every page that can be pushed into the detail stack must apply the modifier itself.
