# GitAgent — Development Notes

> [中文版](Development.zh.md)

Developer-facing records moved out of the README: vision, detailed feature status, architecture decisions, roadmap, and project structure.
...
Design documents: [Agent layer technical roadmap](Roadmap.md) (GitTaskBench-style task execution + in-repo benchmark harness) and [Daily digest technical roadmap](Digest.md) (trending-repo briefing with feedback-driven personalization).
...
│   ├── Development.md        # This file — vision, status, roadmap, structure
...
- iPhone uses Safari-style workspace pages, each with one `NavigationStack`, instead of a collapsed split view: the split view runs two competing back-gesture systems and pops two levels per swipe.
- **Personal data policy:** personal account values (Apple team ID, signing identity, GitHub OAuth Client ID) live only in the two gitignored files (`Local.xcconfig`, `GitAgent/Auth/LocalSecrets.swift`). The tracked `Build.xcconfig` includes `Local.xcconfig` via `#include?` and contains no personal data.
